...
export default function CategoriesSection() {
  const { categories, loading } = useCategories({ withCounts: true });
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        {loading
          ? Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="h-44 rounded-3xl bg-foreground/5 animate-pulse" />
            ))
          : categories.slice(0, 10).map((cat, i) => {
              const g = CATEGORY_GRADIENTS[i % CATEGORY_GRADIENTS.length];
              return (
                <Link key={cat.id} to={`/browse?category=${encodeURIComponent(cat.name)}`} className="group relative bg-card border border-border rounded-3xl p-5 hover:shadow-glass hover:-translate-y-1 transition-all overflow-hidden">
                  ...
                  <span className="mt-3 inline-flex items-center gap-1.5 text-[11px] font-bold text-foreground/70 bg-foreground/5 px-2.5 py-1 rounded-full">
                    <i className="fas fa-eye text-[10px]" /> Browse · {cat.serviceCount ?? 0}
                  </span>
                </Link>
              );
            })}
